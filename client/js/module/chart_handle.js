import { math } from "./data_handle.js";
import { GraphHandle, graphics } from "./graph_handle.js";
...
class BarChart {
    constructor(container, samples, options) {
        ...
        this.canvas.style = "backgroud-color:white;";
        container.append(this.canvas);

        // console.log(container, samples, options);

        this.ctx = this.canvas.getContext("2d");

        this.maxValue = options.maxValue?options.maxValue:Math.max(...Object.values(this.samples));
        this.gridScale = Math.floor(this.maxValue / 5);

        // console.log(this.canvas.width, this.maxValue, this.gridScale);
        this.#draw();
    }
    ...
    #drawGridLines() {
        ...
        while (gridValue <= this.maxValue) {
            ...
            // console.log(canvasActualHeight, this.canvas.height, this.options.padding);

            // console.log(0,gridY,this.canvas.width,gridY,this.options.gridColor);
            GraphHandle.drawLine(this.ctx,0,gridY,this.canvas.width,gridY,this.options.gridColor);
            GraphHandle.drawLine(this.ctx,15,this.options.padding/2,15,gridY+this.options.padding/2,this.options.gridColor);
            ...
            // console.log(gridValue, this.gridScale, this.maxValue);
        }
    }
    #drawBars() {
        const canvasActualHeight = this.canvas.height - this.options.padding * 2;
        ...
        let barIndex = 0;
        const numberOfBars = Object.keys(this.samples).length;
        const barSize = canvasActualWidth / numberOfBars;

        const values = Object.values(this.samples);
        ...
        for (let val of values) {
            ...
            GraphHandle.drawBar(
                ...
                barSize,
                barHeight,
                this.options.colors[barIndex % this.options.colors.length]
            );
    ...
    #drawLegend(legend_container) {
        let pIndex = 0;
        let ul = document.createElement("ul");
        legend_container.append(ul);

        for (let k of Object.keys(this.samples)) {
            let li = document.createElement("li");
            li.style.listStyle = "none";
            li.style.borderLeft = "20px solid " + this.options.colors[pIndex % this.options.colors.length];
