    ...
    async actionData(event, modal, data) {
        ...
        if (action) {
            if (action.handler === 'self') {
                ...
                const component_module = action.handler[action_handle];
...
class View {
    ...
    displayActionbar(actionbar, componentData) {
        ...
        actionbar.forEach(nav_item => {
            ...
            item.data_action_handle = componentData.title_data.order_type;
    ...
    bindActionEvent(handler) {
        this.action_bar.addEventListener('click', event => {
            ...
            const param = {[event.target.name]:event.target.data_id, "action_handle": event.target.data_action_handle};
            // console.log(param);
