...
class Model {
    ...
    async getPageData(param) {
        const q_param = {...param, ...this.route.param};
    ...
    async navbarActionData(event, container, param) {
        ...
        if (action && action.container == 'dialog') {
            ...
            let q_param = {...this.route.param, ...param};
...
class View {
    ...
    async displayContent(componentMeta, componentData) {
        ...
        if (('column_data' in componentData || componentMeta.table_template.fields > 0) && 'table_data' in componentData) {
            if ('column_data' in componentData) {
    ...
    displayNavbar(navbar) {
        ...
        navbar.forEach(nav_item => {
            const item = this.ui_handle.createElement(nav_item.f_type, nav_item.css_class?nav_item.css_class:null);
    ...
    displayTable(tableTemplate, tableData) {
        ...
        tableTemplate.fields.forEach(field => {
            ...
            th.innerText = field[fields_attr.d_name];
            if (field.f_sort) {
        ...
        if (tableData) {
            tableData.forEach(row => {
                ...
                tableTemplate.fields.forEach(field => {
                    ...
                    td.innerText = row[field[fields_attr.f_data_val]]?row[field[fields_attr.f_data_val]]:'';
                    if (fields_attr.f_css_class) {
                        td.classList.add(fields_attr.f_css_class);
                    }
