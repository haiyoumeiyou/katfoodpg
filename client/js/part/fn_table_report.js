...
class View {
    ...
    displayActionbar(actionbar, componentData) {
        ...
        const select_options = 'options' in componentData ? componentData.options :null;
        actionbar.forEach(nav_item => {
            const item = this.ui_handle.createElement(nav_item.f_element);
            ...
            if (nav_item.f_id in this.input_holder && this.input_holder[nav_item.f_id] && nav_item.f_element == "input") {
                item.value = this.input_holder[nav_item.f_id];
            }
            if (nav_item.f_data_key && nav_item.f_data_key in select_options) {
                ...
                const options = select_options[nav_item.f_data_key];
                options.forEach(row => {
                    const option = this.ui_handle.createElement('option');
                    option.textContent = row[nav_item.f_option_text];
                    option.value = row[nav_item.f_option_val];
                    if ((nav_item.f_id in this.input_holder && option.value === this.input_holder[nav_item.f_id]) || (nav_item.f_default_val && option.value === nav_item.f_default_val)) {
                        option.setAttribute('selected', true);
                    }
                    item.appendChild(option);
                });
    ...
    bindActionEvent(handler) {
        this.action_bar.addEventListener('click', event => {
            if (event.target.tagName === 'BUTTON') {
                ...
                let data = this._inputs;
                if ('search' in data && data['search']) {
                    data['search'] = '%' + this._inputs['search'] + '%';
                } 
                console.log(event, this._inputs, data);
                handler(event, container, data);
            }
        });
    }
