...
class Model {
    ...
    async getMeta() {
        ...
        if ('param' in this.componentInfo) {
            this.component_data = this.componentInfo.param;
        }
        this.component_data.device_footprint = UiHandle.getMachineId();
            ...
            if (action.handler === 'self') {
                ...
                if (endpoint) {
                    ...
                    if (endpoint_data[0] === 'ok') {
                        const db_data = endpoint_data[1][0];
                        if ("localStorage" in this.component_meta.modal_template) {
                            for (const [k, v] of Object.entries(this.component_meta.modal_template.localStorage)) {
                                if (v in db_data) {
                                    UiHandle.setLocalStorageItem(k, db_data[v]);
                                }
                            }
                        }
