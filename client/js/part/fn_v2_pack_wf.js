...
class Model {
    ...
    async dataChange(event, modal, data) {
        ...
        // }
        const d_endpoint = await this.router.constructor.metaSectionToDataEndpoint(this.componentInfo.meta_section);
        const component_data = await this.getComponentData(d_endpoint, param);
        this._commit(component_data.data, component_data.status);

        if (action && 'container' in action && action.container == 'dialog') {
            action.param = param;
...
class View {
    constructor(container){
        ...
        this.component.append(this.dialog, this.title_section, this.action_section, this.scroll_table)
    }
    get _inputs() {
        let inputs = {};
...
class Controller {
    constructor(model, view){
        ...
        this.view.bindDialogCloseEvent(this.handleCloseEvent);
    }
